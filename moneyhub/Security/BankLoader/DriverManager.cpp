...
bool CDriverManager::SetKernelContinue(bool b)
{
	...
	unsigned char * Ebuffer=new unsigned char[sizeof(b)+256];
	int packLength;
	packLength=DownloadPack((unsigned char *)&b, sizeof(b), Ebuffer);

	::DeviceIoControl(m_DriverManager.GetDriver(), IOCTL_SET_FILTERRESULT, Ebuffer, packLength, NULL, 0, &dwBytesReturned, NULL);
