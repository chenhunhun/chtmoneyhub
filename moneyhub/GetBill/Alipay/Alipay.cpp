				...
				case 5:
					{
						...
						if(NULL != pbUrl)
						{
							...
							if(url.find(L"lab.alipay.com/user/i.htm") != wstring::npos)
							{
								...
								wstring money;
								size_t nPos = info.find(L"aside-available-amount");
								if(nPos != wstring::npos)
								{
									wstring moneytemp = info.substr(nPos + 23, nPos + 50);
								...
								wstring zhanghu;
								int strsize = wcslen(L"aside-multi-account-trigger");
								nPos = info.find(L"aside-multi-account-trigger");//
								if(nPos != wstring::npos)
								{
