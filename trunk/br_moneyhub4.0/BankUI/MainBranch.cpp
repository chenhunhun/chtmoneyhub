...
bool CMainBranch::PopSetPage(wstring mail)
{
	...
	if (hWnd)
	{
		...
		::SendMessage(hWnd, WM_SETTEXT, WM_SVRCHECKUSER, (LPARAM)mail.c_str());
		return true;
	}
	else
	{
...
//
DWORD WINAPI _threadRefreshCurrency(LPVOID lp)//更新汇率库版本
{
	//Sleep(5 *1000);
	wstring totalurl = CHostContainer::GetInstance()->GetHostName(kExchangeRate);//下载汇率测试文件

	DWORD TestTime = 5000;//5s钟进行测试

	//如果没联网或者链接不上服务器，那么一直测试，每隔30s测试一次
	string info;
	DWORD res = 0;
	info = "";
	res = DownloadCurrency(TestTime, totalurl, info);

	if(res == 0)
		CBankData::GetInstance()->SetCurrency(info);

	return 0;
}

void CMainBranch::RefreshCurrency()
{
	DWORD dw;
	::CloseHandle(CreateThread(NULL, 0, _threadRefreshCurrency, NULL, 0, &dw));
}
...
int CMainBranch::UnInstall()
{
	...
	if (hFile != INVALID_HANDLE_VALUE)
	{
		...
		USES_CONVERSION;
		sprintf_s(szBuf, sizeof(szBuf), "%s?Moneyhubuid=%s&V=%s&Sn=%s",W2CA(CHostContainer::GetInstance()->GetHostName(kUninstall).c_str()), GenHWID2().c_str(), ProductVersion_All,CSNManager::GetInstance()->GetSN().c_str());
