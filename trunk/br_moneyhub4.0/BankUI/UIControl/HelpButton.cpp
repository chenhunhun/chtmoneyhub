...
LRESULT CHelpButton::OnClick(int nIndex, POINT pt)
{
	...
	USES_CONVERSION;
	sprintf_s(szBuf, sizeof(szBuf), "%s?Moneyhubuid=%s&V=%s&Sn=%s",W2CA(CHostContainer::GetInstance()->GetHostName(kHelp).c_str()), GenHWID2().c_str(), ProductVersion_All,CSNManager::GetInstance()->GetSN().c_str());
