...
#define MSG_WM_PAINT(func) \
	...
	{ \
		...
		{ \
			PAINTSTRUCT paint; \
			CDCHandle dc(::BeginPaint(hWnd, &paint)); \
			func(dc, paint.rcPaint); \
			::EndPaint(hWnd, &paint); \
		} \
		lResult = 0; \
		if(IsMsgHandled()) \
...
// void OnMDICreate(LPCTSTR lpName, DWORD_PTR dwData)
#define MSG_WM_MDICREATE(func) \
	if (uMsg == WM_MDICREATE) \
	{ \
		SetMsgHandled(TRUE); \
		func((LPCTSTR)wParam, (DWORD_PTR)lParam); \
		lResult = 0; \
		if(IsMsgHandled()) \
			return TRUE; \
	}
...
// void OnMDIDestroy(CWindow wndDestroy)
#define MSG_WM_MDIDESTROY(func) \
...
#define COMMAND_HANDLER_EX(id, code, func) \
	if (uMsg == WM_COMMAND && (WORD)code == HIWORD(wParam) && (WORD)id == LOWORD(wParam)) \
	{ \
		SetMsgHandled(TRUE); \
		func((UINT)HIWORD(wParam), (int)LOWORD(wParam), (HWND)lParam); \
		lResult = 0; \
...
#define COMMAND_CODE_HANDLER_EX(code, func) \
	if (uMsg == WM_COMMAND && (WORD)code == HIWORD(wParam)) \
	{ \
		SetMsgHandled(TRUE); \
		func((UINT)HIWORD(wParam), (int)LOWORD(wParam), (HWND)lParam); \
		lResult = 0; \
...
#define HANDLE_TUO_SETTEXT() \
	if (uMsg == WM_SETTEXT) \
	{ \
		ProcessWindowMessage(hWnd, wParam, 0, lParam, lResult); \
		return TRUE; \
	}

#define HANDLE_TUO_COPYDATA() \
	if (uMsg == WM_COPYDATA) \
	{ \
		PCOPYDATASTRUCT pcds = (PCOPYDATASTRUCT)lParam; \
		ProcessWindowMessage(hWnd, pcds->dwData, pcds->cbData, (LPARAM)pcds->lpData, lResult); \
		return TRUE; \
	}

#define HANDLE_TUO_COPYDATA_WITH_WPARAM() \
	if (uMsg == WM_COPYDATA) \
	{ \
		PCOPYDATASTRUCT pcds = (PCOPYDATASTRUCT)lParam; \
		ProcessWindowMessage(hWnd, pcds->dwData, wParam, (LPARAM)pcds->lpData, lResult); \
		return TRUE; \
	}

#define FORWARD_MSG_TO_MEMBER(msg, member) \
	if (uMsg == msg) \
	{ \
		member.ProcessWindowMessage(hWnd, msg, wParam, lParam, lResult); \
		return TRUE; \
	}

#define TIMER_HANDLER(timer, func) \
	if(uMsg == WM_TIMER && wParam == timer) \
	{ \
		bHandled = TRUE; \
		func(); \
		lResult = 0; \
...
#define TIMER_ONCE_HANDLER(timer, func) \
	if(uMsg == WM_TIMER && wParam == timer) \
	{ \
...
// LRESULT OnReflectedNotifyHandlerEX(LPNMTOOLBAR pnmh)
#define REFLECTED_TOOLBAR_NOTIFY_HANDLER_EX(item, cd, func) \
	if (uMsg == OCM_NOTIFY && cd == ((LPNMHDR)lParam)->code && item == ((LPNMTOOLBAR)lParam)->iItem) \
	{ \
		SetMsgHandled(TRUE); \
		lResult = func((LPNMTOOLBAR)lParam); \
		if(IsMsgHandled()) \
			return TRUE; \
	}
