...
public:
	...
	LRESULT OnTimer(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL &bHandled)
	{
		...
		{
			//m_bmpSpinner.ShowWindow(SW_HIDE);	
		...
		if(wParam == MH_GIF_REFRESH)
		{
			...
			gh.DrawImage(m_SpinnerGif, m_rcMsgBoxSpinner.left, m_rcMsgBoxSpinner.top,m_SpinnerGif->GetWidth(),m_SpinnerGif->GetHeight());
			//重新设置当前的活动数据帧
	...
	LRESULT OnInitDialog(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		// SetTimer(MH_GIF_REFRESH, 100 ,NULL);
		//m_bmpSpinner.Create(m_hWnd, m_rcMsgBoxSpinner, NULL, WS_VISIBLE | WS_CHILD | WS_CLIPSIBLINGS | WS_CLIPCHILDREN, 0);
