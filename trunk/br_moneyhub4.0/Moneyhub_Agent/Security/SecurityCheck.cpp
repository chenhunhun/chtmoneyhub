...
void CSecurityCheck::Start(int bCheckType)
{
	...
	DWORD dw;
	m_evSysVerifyWithNet = CreateEvent(NULL, false, false, L"NETSYSVERIFY");
	m_evSysVerifyNoNet = CreateEvent(NULL, false, false, L"NONETSYSVERIFY");
	m_evCloudVerify = CreateEvent(NULL, false, false, L"CLOUDVERIFY");

	m_evSysVerifyWithNetBack = CreateEvent(NULL, false, false, L"NETSYSBACK");
	m_evSysVerifyNoNetBack = CreateEvent(NULL, false, false, L"NONETSYSBACK");
	m_evCloudVerifyBack = CreateEvent(NULL, false, false, L"CLOUDBACK");

	m_reNetSysVerify = true;
	m_reSysVerify = true;
	m_reCloudVerify = KSAFE_S_R_UNKNOWN;
	...
	CloseHandle(CreateThread(NULL, 0, _threadNetSysVerify, this, 0, &dw));
	CloseHandle(CreateThread(NULL, 0, _threadSysVerify, this, 0, &dw));
	CloseHandle(CreateThread(NULL, 0, _threadCloudVerify, this, 0, &dw));
}
DWORD WINAPI CSecurityCheck::_threadNetSysVerify(LPVOID lp)//有网微软检测线程
{
	CSecurityCheck* pThis = (CSecurityCheck *)lp;

	while( 1 )
	{
		::WaitForSingleObject(pThis->m_evSysVerifyWithNet, INFINITE);

		if(pThis->m_wsNetSysVerifyFile.size() > 0)
		{
			pThis->m_reNetSysVerify = CCloudCheckor::GetCloudCheckor()->FileVerify(pThis->m_wsNetSysVerifyFile, true);
		}
		SetEvent(pThis->m_evSysVerifyWithNetBack);

	}

}

DWORD WINAPI CSecurityCheck::_threadSysVerify(LPVOID lp)//无网微软检测线程
{
	CSecurityCheck* pThis = (CSecurityCheck *)lp;

	while( 1 )
	{
		::WaitForSingleObject(pThis->m_evSysVerifyNoNet, INFINITE);

		if(pThis->m_wsSysVerifyFile.size() > 0)
		{
			pThis->m_reSysVerify = CCloudCheckor::GetCloudCheckor()->FileVerify(pThis->m_wsSysVerifyFile, false);
		}
		SetEvent(pThis->m_evSysVerifyNoNetBack);

	}
}

DWORD WINAPI CSecurityCheck::_threadCloudVerify(LPVOID lp)//云查杀线程
{
	CSecurityCheck* pThis = (CSecurityCheck *)lp;

	while( 1 )
	{
		::WaitForSingleObject(pThis->m_evCloudVerify, INFINITE);

		if(pThis->m_wsCloudVerifyFile.size() > 0)
		{
			pThis->m_reCloudVerify = CCloudCheckor::GetCloudCheckor()->FileScan(pThis->m_wsCloudVerifyFile);
		}
		SetEvent(pThis->m_evCloudVerifyBack);
...
DWORD WINAPI CSecurityCheck::_threadCheckAuto(LPVOID lp)
{
	...
	{
		set<wstring> tList,sList;
		Sleep(5 * 1000);//每隔15s中检查一次灰名单和云查杀名单
...
#define RECONFIGTIME 3 * 60 * 60* 1000
DWORD WINAPI CSecurityCheck::_threadSelfCommunicate(LPVOID lp)
{
	CRecordProgram::GetInstance()->RecordCommonInfo(MY_PRO_NAME, MY_THREAD_DRIVER_COM, L"开启通信进程");
	...
	DWORD pre, lastTM;
	lastTM = 0;
	bool g_bRevokeCheck = true;
	bool g_bCloudVerify = true;
	while(true)
	{
		//CRecordProgram::GetInstance()->RecordCommonInfo(L"DriverTest", MY_THREAD_DRIVER_COM, L"等待");
		WaitForSingleObject(hCommEvent, INFINITE);
		//CRecordProgram::GetInstance()->RecordCommonInfo(L"DriverTest", MY_THREAD_DRIVER_COM, L"等待开始");
		pre = GetTickCount();

		if((pre - lastTM) > RECONFIGTIME)//3小时重置
		{
			g_bRevokeCheck = true;
			g_bCloudVerify = true;			
		}
		...
		if(cd.GetGrayFile(files))
		{
			CRecordProgram::GetInstance()->RecordCommonInfo(L"DriverTest", MY_THREAD_DRIVER_COM, L"获得文件");
			set<wstring>::iterator ite = files.begin();
			...
			for(;ite != files.end();ite ++)
			{
				...
				//CRecordProgram::GetInstance()->RecordCommonInfo(L"DriverTest", MY_THREAD_DRIVER_COM, CRecordProgram::GetInstance()->GetRecordInfo(L"接收到文件检测：%s",filePath.c_str()));
				...
				bool bSysVerifyResult = false;
				if(g_bRevokeCheck == true)
				{
					pThis->m_wsNetSysVerifyFile = filePath;
					SetEvent(pThis->m_evSysVerifyWithNet);
					DWORD veriRes = ::WaitForSingleObject(pThis->m_evSysVerifyWithNetBack, 5000);
					if(veriRes == WAIT_TIMEOUT)
					{
						lastTM = GetTickCount();
						CRecordProgram::GetInstance()->RecordCommonInfo(MY_PRO_NAME, MY_THREAD_CATHE,L"签名超时");

						g_bRevokeCheck = false;
					}
					else
						bSysVerifyResult = pThis->m_reNetSysVerify;
				}
				else
				{
					pThis->m_wsSysVerifyFile = filePath;
					SetEvent(pThis->m_evSysVerifyNoNet);
					DWORD veriRes = ::WaitForSingleObject(pThis->m_evSysVerifyNoNetBack, INFINITE);
					bSysVerifyResult = pThis->m_reSysVerify;
				}

				KSAFE_RESULT kResult;
				if( !bSysVerifyResult )
				{
					if(g_bCloudVerify == true)
					{
						pThis->m_wsCloudVerifyFile = filePath;
						SetEvent(pThis->m_evCloudVerify);
						DWORD veriRes = ::WaitForSingleObject(pThis->m_evCloudVerifyBack, 5000);
						if(veriRes == WAIT_TIMEOUT)
						{
							CRecordProgram::GetInstance()->RecordCommonInfo(MY_PRO_NAME, MY_THREAD_CATHE,L"云查杀超时");
							g_bCloudVerify = false;
							kResult = KSAFE_S_R_UNKNOWN;
						}
						else
							kResult = pThis->m_reCloudVerify;
				...
				EnterCriticalSection(&pThis->m_cs);
				//CRecordProgram::GetInstance()->RecordCommonInfo(L"DriverTest", MY_THREAD_DRIVER_COM, CRecordProgram::GetInstance()->GetRecordInfo(L"得到文件检测结果：%s",filePath.c_str()));
				if( (bSysVerifyResult) || (KSAFE_S_R_SAFE == kResult) )
				{
					CRecordProgram::GetInstance()->RecordCommonInfo(MY_PRO_NAME, MY_THREAD_CATHE, CRecordProgram::GetInstance()->GetRecordInfo(L"云和签名验证结果%s",filePath.c_str()));
					bSysVerifyResult = true;
					CGlobalData::GetInstance()->GetWhiteFileList()->insert(filePath);
				}
				else if(   KSAFE_S_R_UNKNOWN == kResult )
				{
					//CRecordProgram::GetInstance()->RecordCommonInfo(MY_PRO_NAME, MY_THREAD_CATHE, CRecordProgram::GetInstance()->GetRecordInfo(L"未验证结果%s",filePath.c_str()));
					bSysVerifyResult = true;
				...
				USES_CONVERSION;
				CDriverCommunicator cd;
				//CRecordProgram::GetInstance()->RecordCommonInfo(L"DriverTest", MY_THREAD_DRIVER_COM, CRecordProgram::GetInstance()->GetRecordInfo(L"驱动设置开始：%s",filePath.c_str()));
				
				cd.SetKernelContinue(bSysVerifyResult);
