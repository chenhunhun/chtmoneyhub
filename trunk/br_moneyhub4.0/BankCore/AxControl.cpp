...
CAxControl::CAxControl(HWND hChildFrame) : m_hChildFrame(hChildFrame), m_isGetBill(false),m_time(0), m_nErrCode(0)
{
	// 事件回调对象
...
{/*
	std::vector<std::string> paramVec;
	USERSTATUS emParam = (USERSTATUS)lParam;
	if (emNotLoad != emParam)
...
LRESULT CAxControl::OnCallJSShowText(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	// 设置要显示的错误码
	if (wParam > 0)
		m_nErrCode = (int)wParam;

	// 如果页面已经加载完成并且有错误信息要显示
	if (wParam == 0 && m_nErrCode > 0)
	{
		std::vector<std::string> paramVec;
		char chBuf[20] = {0};

		string strTp = itoa((int)m_nErrCode, chBuf, 10);
		paramVec.push_back(strTp);
		CallJScript("autoLoadMessage", paramVec);
		m_nErrCode = 0;
	}
	return 0;
}
...
{
	CBankData::GetInstance()->m_CurUserInfo.emChangeStatus = (CHANGESTATUS)lParam;
	return 0;
}
...
LRESULT CAxControl::OnInitSynchroBtnStatus(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	CExternalDispatchImpl::m_sbUpdateSynchroBtn = false;
	return 0;
}