...
CUserBehavior::CUserBehavior()
{
	...
	*(_tcsrchr(szPath, '\\') + 1) = 0;
	...
	m_url = CHostContainer::GetInstance()->GetHostName(kPDataFeedback);
...
void CUserBehavior::Action_Install(int i)
{
	...
	CPostData::getInstance()->sendData(m_url.c_str(), (LPSTR)(LPCSTR)sstr.c_str(), sstr.size(), 5000);
}
...
void CUserBehavior::Action_Uninstall(int i)
{
	...
	CPostData::getInstance()->sendData(m_url.c_str(), (LPSTR)(LPCSTR)xml.c_str(), xml.size(), 5000);
}
// 启动反馈（运行反馈）的函数 需要等
...
void CUserBehavior::Action_ProgramExit()
{
	...
	CPostData::getInstance()->sendData(m_url.c_str(), (LPSTR)(LPCSTR)xml.c_str(), xml.size(),3000);
}
