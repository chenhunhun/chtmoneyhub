...
void CStatusBarSkin::DrawDownloadProgress(CDCHandle dc, const RECT& rect, DWORD dwProgress)
{
	int iWidth = m_bmpStatusProgress.GetWidth();
	int iHeight = m_bmpStatusProgress.GetHeight();

	int nDescHight = rect.bottom - rect.top;
	int nDescWhidth = rect.right - rect.left;
	RECT rcDest = { rect.left, rect.top, rect.left + nDescWhidth, rect.top + iHeight };
	CSkinManager::DrawExtends(dc, rcDest, m_bmpStatusProgress, 0, iWidth);

	RECT rcSrcOverlay = { 0, 0, 9,  8};
	RECT rcDestCenter = { 0, 0, dwProgress * (nDescWhidth - 19) / 100 + 9, 8 };
	OffsetRect(&rcDestCenter, rect.left + 5, rect.top + 4);
	CSkinManager::DrawExtends(dc, rcDestCenter, m_bmpStatusProgress2, 0, 9);
}
