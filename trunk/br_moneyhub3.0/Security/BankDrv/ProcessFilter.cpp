...
BOOL AddProtectPID(ULONG PID)
{
	...
	if( PID == 0 )
	{
		...
		g_iNoPassNum = 0;
		WriteSysLog(LOG_TYPE_INFO,L" MoneyHub.exe is start!");
	}
	else
	{
		//DbgPrint("add PID is: %d\n", PID);
	...
	if(Create == TRUE)
	{
		...
		{		
			if(SetReg(AUTORUN,REGNAME) == false)
				WriteSysLog(LOG_TYPE_INFO,L" is userinital create reg : %s","error");
		}
...
bool IsInstall(bool bCheck)
{
	bool bReturn = false;

	if( g_ProcessFilterCount == 1 && g_iNoPassNum > 5 )
		bReturn = true;
	else if( g_ProcessFilterCount == 1 )
	{	
		if( bCheck )
			g_iNoPassNum ++;
	}
