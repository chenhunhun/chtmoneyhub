...
int CSecurityCheck::CheckMoneyHubList(unsigned char *buffer, int length, const wchar_t *path, wchar_t *message)
{
	...
	while (pos<length)
	{
		...
		if (hFile != INVALID_HANDLE_VALUE)
		{
			...
			{
				CRecordProgram::GetInstance()->RecordWarnInfo(L"CSecurityCheck", 3012, strFile.c_str());
		...
		{
			CRecordProgram::GetInstance()->RecordWarnInfo(L"CSecurityCheck", 3013, strFile.c_str());
...
int CSecurityCheck::VerifyMoneyHubList(const char *path, const char* CHKFileName, wchar_t *message)
{
	...
	if (hFile != INVALID_HANDLE_VALUE)
	{
		...
		{
			USES_CONVERSION;
			CRecordProgram::GetInstance()->RecordWarnInfo(L"CSecurityCheck", 3012, A2W(CHKFileName));
			wcscpy_s(message, MSG_BUF_LEN, L"您的财金汇客户端已被破坏，请重新下载安装，建议您对电脑进行全面的病毒扫描");
			return -3003;
		}
...
int CSecurityCheck::VerifyUrlList(const char* lpCHKFileName, wchar_t *message)
{
	...
	if (hFile != INVALID_HANDLE_VALUE)
	{
		...
		{
			USES_CONVERSION;
			CRecordProgram::GetInstance()->RecordWarnInfo(L"CSecurityCheck", 3012, A2W(lpCHKFileName));
			wcscpy_s(message, MSG_BUF_LEN, L"您的财金汇客户端已被破坏，请重新下载安装，建议您对电脑进行全面的病毒扫描");
			return -3003;
		}
