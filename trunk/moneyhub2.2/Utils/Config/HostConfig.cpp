...
bool CHostContainer::GetAllHostName(VECTORNPBNAME *pVvecNPB )
{
	...
	///add by bh for get no hookProtect bank name
	...
	if( pNPB && pVvecNPB)
	{
			...
			Binary; Binary = Binary->NextSiblingElement())
		{
			...
			if(IDAttribute)
			{
				pVvecNPB->push_back(A2W(IDAttribute->Value()));
			}
			//
			/*if(Binary->Attribute("classname") )
				;*///MessageBoxA(NULL,Binary->Attribute("classname"),"",MB_OK);
			//
 		}
	}
