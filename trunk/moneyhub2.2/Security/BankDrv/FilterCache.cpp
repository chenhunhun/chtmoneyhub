...
PPROCESS_FILTERS InsertProcessFilter(ULONG PID)
{
	...
	KdPrint(("InsertProcessFilter. PID: %d  Return: %08x\n", PID, ProcessFilter));
	return ProcessFilter;
}
...
bool IsInBlackCache(const PUNICODE_STRING filepath)
{
	...
	while(current != NULL)
	{
		...
		if(count > 500)  
		{
			WriteSysLog(LOG_TYPE_DEBUG,L"elapse the large timer 500 counter\n ");
			break;
		}
...
{
	int iRow = 0;
	for(PPROCESS_FILTERS p=g_ProcessFilters.next  ;p; p = p->next )
	{
		for(PFILTER_ITEM q=p->RootFilter ;q;  q = q->next)
		{
			wchar_t buf[500];
			if(q->FilePath.Length < 499 )
			{
				RtlCopyMemory(buf,q->FilePath.Buffer,q->FilePath.Length);
				*((wchar_t *)(buf + q->FilePath.Length)) = L'\0';
