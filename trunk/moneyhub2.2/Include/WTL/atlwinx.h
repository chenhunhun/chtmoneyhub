...
static BOOL CheckIfParent(HWND hTestWnd, HWND hGrandParentWnd)
{
	hTestWnd = ::GetParent(hTestWnd);
	while (hTestWnd)
	{
		if (hTestWnd == hGrandParentWnd)
			return TRUE;
		hTestWnd = ::GetParent(hTestWnd);
	}
	return FALSE;
...
static HWND GetNextVisibleWindow(HWND hWnd)
{
	HWND hNext = hWnd;
	while (hNext)
	{
...
static LRESULT SendMessageBlock(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	DWORD dwRes = 0;
	::SendMessageTimeout(hWnd, uMsg, wParam, lParam, SMTO_BLOCK | SMTO_NOTIMEOUTIFNOTHUNG, 0, &dwRes);
	return dwRes;
}
