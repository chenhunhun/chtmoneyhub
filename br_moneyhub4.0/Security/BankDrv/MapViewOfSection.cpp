...
bool g_bBusy		 = false;//表明正在进行loadlibrary
...
// 	}	
	...
	if( bOK )
	{
		if(!IsInBlackCache(filename))
		{
			WriteSysLog(LOG_TYPE_INFO,L"F:%s", filename->Buffer);
			AddBlackCache(filename);
		}
		bReturn = false;
	}
	else if( !IsHashSecure(final) )//传递到上层 
	{
		if( !IsInstall() )
		{
			...
			if(!bReturn)
				WriteSysLog(LOG_TYPE_INFO,L"%ws:%ws", filename->Buffer, bReturn? L"P":L"O");
			if( bReturn )
...
VOID UnHookSSDT()
{
	WriteSysLog(LOG_TYPE_DEBUG,L" UnT");
	...
	WriteSysLog(LOG_TYPE_DEBUG,L" FiT");
