...
private:
	...
	// 增加互斥量句柄成员变量，在需要的时候进行同步 郑鹏 2011.2.14 16:34 Add Begin
	HANDLE m_hFileMutex;
	// 增加互斥量句柄成员变量，在需要的时候进行同步 郑鹏 2011.2.14 16:34 Add End
public:
	...
	*/
	...
	// 根据振兴的想法，给该函数添加bool变量，让调用者决定是否进行同步  郑鹏 2011.2.14 16:26 修改 Begin
	void LogInitialize(int level,wchar_t* logdirectory = L".\\Logs",wchar_t* logfilename = L"log.d",long size = 10000000,int strategy = 2,bool bSynchronize = false);
	// 根据振兴的想法，给该函数添加bool变量，让调用者决定是否进行同步  郑鹏 2011.2.14 16:26 修改 End

	/**
	* 写日志函数
	* @param iLevel:int,要写入日志信息的等级，请参考LogConst.h文件内等级。
	* @param format:wchar_t*,输入信息的格式，此格式参考printf。
	* @param ...:变参,参考printf，要写入的信息，为Unicode格式。
	* 写日志信息接口，写日志信息大小请不要超过规定长度的大小！最大暂时为256字符！！
	*/
