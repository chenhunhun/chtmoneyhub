...
void CSepButton::CreateButton(HWND hParent, LPCTSTR lpName, UINT nBtnClickMessage)
{
	...
	//m_nMesWParam = nMesWParam;

	HWND hWnd = Create(hParent, CRect(0, 0, m_bitmap.GetWidth(), m_bitmap.GetHeight()), NULL, WS_CHILD | WS_VISIBLE);
	ATLASSERT(hWnd);

	//if (0 != m_nMessage && 0 != nMesWParam)
	//	::PostMessage(hParent, m_nMessage, nMesWParam, SW_SHOW);
}
...
LRESULT CSepButton::OnMouseMove(int nIndex, POINT pt)
{
	if (0 != m_nMessage)
		::SendMessage(m_hParentWnd, m_nMessage, 0, SW_SHOW);
