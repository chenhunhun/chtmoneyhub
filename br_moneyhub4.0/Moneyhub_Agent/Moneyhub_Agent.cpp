...
//DWORD WINAPI _threadRefreshCurrency(LPVOID lp);
...
//	HRESULT hRes = ::CoInitializeEx(NULL, COINIT_MULTITHREADED);
	...
	Sleep(5 * 1000);//延后5s检测防止安全检测线程未开启导致点击不成功
...
DWORD TestDownloadMode(DWORD dwSendTimeOut, wstring& tUrl, bool bBreakPoint)
{
	CUrlCrack url;
...
DWORD WINAPI _threadTestDownloadMode(LPVOID lp)//测试下载模式的函数
{
	...
	while( 1 )
	{
		res = TestDownloadMode(TestTime, totalurl, true);
	...
	while( 1 )
	{
		res = TestDownloadMode(TestTime, totalurl, false);
		...
		else
		{
			if( i = 0)
				...
				return 0;
			res = TestDownloadMode(TestTime, totalurl, true);//再测试断点续传
			if(res == 0)
			{
				CDownloadMode::GetInstance()->SetMode(eHttpBreak);
				return 0;
