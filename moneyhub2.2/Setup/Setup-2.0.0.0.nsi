...
completed3:
	...
	File /r /x .svn /x *.lib /x *.exp /x *.pdb /x *.ilk /x *.map /x BankActiveXTest.html /x BankLoaderTester.exe /x DigsigGen.exe /x ModuleVerifierTester.exe /x VerifyList.xml /x info.xml /x CloudCheck.ini /x syslist.txt /x BlackList.txt /x usbkeyinfo.xml /x BankInfo ..\_Bin\Debug\*.*
	;开放权限，允许任何人读写，以保证不提权
