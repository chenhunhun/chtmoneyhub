...
HRESULT CWebBrowserEventsManager::OnNewWindow(LPCTSTR lpszUrl, LPCTSTR lpszReferer, IDispatch **&ppDisp, VARIANT_BOOL *&Cancel)
{
	...
	const CWebsiteData *pData = CURLList::GetInstance()->GetData(strDomain.c_str(), lpszUrl);
...
*/
	...
	if (CListManager::IsHomePage(lpszReferer))
	{
	}
	...
	if (hChildFrame == NULL)
	{
		if (bOpenBackground)
		{
			hChildFrame = (HWND)CreateNewPage_3_NewWindow(m_pAxControl->m_hChildFrame, lpszUrl, FALSE, m_pAxControl->m_hChildFrame);
			m_bForceBackOpen = FALSE;
		}
		else
			hChildFrame = (HWND)CreateNewPage_3_NewWindow(m_pAxControl->m_hChildFrame, lpszUrl, TRUE, m_pAxControl->m_hChildFrame);
	}
	...
	if (hAxControl != NULL)
	{
		...
		{		
			...
			//size_t nvidName = sUrl.find(L"bankofbeijing.com.cn");
			//if(nvidName != wstring::npos)
			//	delay = 1;
			//// 华夏银行的直接关闭
			//nvidName = sUrl.find(L"ebank.hxb.com.cn/HxPer/basPreLogin.do");
			//if(nvidName != wstring::npos)
			//	delay = 1;
			IStream* pStream = (IStream*)::SendMessageBlock(hAxControl, WM_AX_GET_WEBBROWSER2_CROSS_THREAD, delay, 0) ;
			if (pStream)
			{
...
HRESULT CWebBrowserEventsManager::OnBeforeNavigate2(REFIID riid, LCID lcid, WORD wFlags, DISPPARAMS* pDispParams, VARIANT* pVarResult, EXCEPINFO* pExcepInfo, UINT* puArgErr)
{
	...
	if (IsToppestWindowOfBrowser2(pDispParams->rgvarg[6].pdispVal))
	{
		...
		if (CListManager::IsHomePage(strUrl))
		{
		}
		else if (NULL == CURLList::GetInstance()->GetData(NULL, strUrl))
		{
		...
		//CThreadRequirement::GetCurrentThreadReq()->UpdateURL(m_cstrLastUrl);
		...
		if (SUCCEEDED(hr) && spDispDoc)
		{
			...
			if (SUCCEEDED(hr) && spHTMLDocument2)
			{
				...
				if (fs.good())
				{
					...
					//size_t n; = strContent.find("$ERRCODE1$");
					//if (n != std::string::npos)
					//{
					//	std::stringstream ssnum;
					//	ssnum << (int)pDispParams->rgvarg[1].lVal;
