...
#include "../../Tools/Logger(Unicode)//LogConst.h"
...
void CCloudCheckor::SetFiles(set<wstring>* pfiles,HWND frame)
{
	m_files = pfiles;
	m_frame = frame;
}
...
bool CCloudCheckor::Initialize()
{
	// 增加判断网络状态的处理
	if(m_log)
		m_log->WriteSysLog(LOG_TYPE_INFO,L"%s",L"开始进行连baidu");
	...
	if(m_log)
		m_log->WriteSysLog(LOG_TYPE_INFO,L"%s",L"开始进行连加载clouddll");
	...
	if(m_log)
		m_log->WriteSysLog(LOG_TYPE_INFO,L"%s",L"加载clouddll完成");
	return true;
}
...
bool CCloudCheckor::SerialScan()
{
	...
	//m_passfiles.
	if(m_frame)
		::PostMessageW(m_frame,WM_CLOUDCHECK,0,0);
	if(m_pshow)
		m_pshow->Update(0);
	HRESULT hr;
	KSAFE_RESULT Result;
	set<wstring>::iterator ite;
	int cloudcheckcount = 0;
	int i = -1;
	int exceedtime = 0;
	for(ite = m_files->begin();ite != m_files->end();ite ++)
	{
		...
		if(exceedtime >= 3)
		{
			::MessageBoxW(NULL,L"网络链接超时，请检测网络！",L"云查杀",MB_OK | MB_SETFOREGROUND);
			return false;
		}

		if(m_log)
			m_log->WriteSysLog(LOG_TYPE_INFO,L"%s:%d",(*ite).c_str(),Result);
	...
	if(m_log)
	{
		m_log->WriteSysLog(LOG_TYPE_DEBUG,L"文件数量:%d",m_files->size());
		m_log->WriteSysLog(LOG_TYPE_DEBUG,L"经过云查杀数量:%d",cloudcheckcount);
	}
	if(m_pshow)
		m_pshow->Update(100);
