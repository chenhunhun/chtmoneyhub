...
#define _DownLoadSubURL _T("downinstall.php")
...
void CBkInfoDownloadManager::MyBankCtrlDownload(LPSTR lpBankID)
{
	...
	std::wstring downPath = CHostContainer::GetInstance()->GetHostName(kDownload) + _DownLoadSubURL;
...
DWORD CBkInfoDownloadManager::CheckThreadProc(LPVOID lpParam)
{
	...
	while(true)
	{
		...
		for (cstIt = pTemp->m_BankManager.begin (); cstIt != pTemp->m_BankManager.end (); cstIt ++)
		{
			...
			CDownloadAndSetupThread* pDlThread = pFinish->ReadAFinishDownloadThread ();
			...
			if (NULL != CNotifyFavProgress::GetInstance()->m_hwndNotify && NULL != pFinish) // 获取并发送进度
			{
				...
				// 银行控件写入到银行数据库中
				if (emBkActiveX & emMyKind)
					CBankData::GetInstance ()->SaveFav ((*cstIt).first, nPercent);
				else if(emUsbKey & emMyKind)
					CBankData::GetInstance ()->UpdateUSB (pDlThread->m_dlUSBParam.nVid, pDlThread->m_dlUSBParam.nPid, pDlThread->m_dlUSBParam.nMid, nPercent);
				// 如果是USBKEY的话得另外设置进度
