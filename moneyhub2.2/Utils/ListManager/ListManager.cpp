...
CListManager::CListManager(bool bBankCore) :m_bBankCore(bBankCore), m_hRegQueryMutex(NULL)// gao 2010-12-13
{
	...
	if(pReq != NULL)
	{
		if (m_bBankCore)
		{
			...
			for (const TiXmlNode *pRequire = pReq->FirstChild("require"); pRequire != NULL; pRequire = pReq->IterateChildren("require", pRequire))
			{
				...
				pWebsiteData->m_Requirements.insert(std::make_pair(pReq->GetID(), pReq));
			}

			m_WebsiteData.insert(std::make_pair(L"www.moneyhub.com.cn", pWebsiteData)); // 插入
		}
	}
	...
	if (!traverseBankInfo((LPWSTR)wcsPath.c_str()))
	{
		if (m_bBankCore)
			...
			CRecordProgram::GetInstance()->FeedbackError(MY_ERROR_PRO_UI, ERR_XML_FILE, L"CListManager中traverseBankInfo错误！");
...
const CWebsiteData* CListManager::GetData(LPCTSTR /*lpszDomain*/, LPCTSTR lpszUrl) const
{
	CString strClearUrl = ExtractClearDomain(lpszUrl);
...
void CListManager::InstallWebsiteRequirements(LPCTSTR lpszID)
{
	for (WebDataMap::iterator it = m_WebsiteData.begin(); it != m_WebsiteData.end(); it++)
	{
		if (_tcsicmp(it->second->GetID(), lpszID) == 0)
		{
			it->second->InstallWebsiteRequirements();
			return;
		}
	}
}
...
CTuoImage CWebsiteData::m_bmpHomeLogo;
CTuoImage CWebsiteData::m_bmpHomeLogo_gray;
...
void CWebsiteData::StartUAC(std::wstring appid)
{
	...
	//PostMessageW(hPop, WM_MONEYHUB_UAC,  0,  pam);
...
void CWebsiteData::InstallWebsiteRequirements()
{
	for (ReqMap::iterator it = m_Requirements.begin(); it != m_Requirements.end(); it++)
		it->second->InstallRequirement();
}
...
bool CListManager::AddANewFavBank(LPWSTR lpParam, bool bBankName)
{
	...
	{
		if (m_bBankCore)
			CRecordProgram::GetInstance()->FeedbackError(MY_ERROR_PRO_CORE, ERR_XML_FILE, CRecordProgram::GetInstance()->GetRecordInfo(L"AddANewFavBank中GetRootXMLFile %s 发生异常！", strPath.c_str()));
		else
			CRecordProgram::GetInstance()->FeedbackError(MY_ERROR_PRO_UI, ERR_XML_FILE, CRecordProgram::GetInstance()->GetRecordInfo(L"AddANewFavBank中GetRootXMLFile %s 发生异常！", strPath.c_str()));
	...
	{
		if (m_bBankCore)
			CRecordProgram::GetInstance()->FeedbackError(MY_ERROR_PRO_CORE, ERR_XML_FILE, CRecordProgram::GetInstance()->GetRecordInfo(L"AddANewFavBank中 %s 中没有根结点！", strPath.c_str()));
		else
			CRecordProgram::GetInstance()->FeedbackError(MY_ERROR_PRO_UI, ERR_XML_FILE, CRecordProgram::GetInstance()->GetRecordInfo(L"AddANewFavBank中%s 中没有根结点！", strPath.c_str()));
		return false;
	}
	...
	for (const TiXmlNode *pCategory = pRoot->FirstChild("category"); pCategory != NULL; pCategory = pRoot->IterateChildren("category", pCategory))
	{
		...
		else
		{
			if (m_bBankCore)
				...
				CRecordProgram::GetInstance()->FeedbackError(MY_ERROR_PRO_UI, ERR_XML_FILE, L"AddANewFavBank中对XML文件内容进行遍历时异常！");
...
bool CListManager::DeleteAFavBank(string appid)
{
	...
	CWebsiteData* pfWebsiteData = NULL;
	m_pFavBkOper->UpDateFavBankID(appid, false);
	RegQueryLock ();
	WebDataMap::const_iterator it = m_WebsiteData.begin();
	for (; it != m_WebsiteData.end(); )
	{
		CWebsiteData* pWebsiteData = it->second;
		if(pWebsiteData != NULL)
		{
			if(wcscmp(pWebsiteData->GetID(),strBankName.c_str()) == 0)
			{
				pfWebsiteData = pWebsiteData;
				it = m_WebsiteData.erase(it);
			}
			else
				it ++;
		}
	...
	RegQueryUnlock ();
	if(pfWebsiteData != NULL)
		delete pfWebsiteData;
...
CTuoImage& CWebsiteData::GetHomeLogo(bool bSelected)
{
	if (bSelected)
	{
		if (m_bmpHomeLogo.IsNull())
		{
			TCHAR szPath[MAX_PATH];
			_stprintf_s(szPath, _T("%s\\Config\\UILogos\\homepage.png"), ::GetModulePath());
			m_bmpHomeLogo.LoadFromFile(szPath);
		}

		return m_bmpHomeLogo;
	}
	else
	{
		if (m_bmpHomeLogo_gray.IsNull())
		{
			TCHAR szPath[MAX_PATH];
			_stprintf_s(szPath, _T("%s\\Config\\UILogos\\homepage2.png"), ::GetModulePath());
			m_bmpHomeLogo_gray.LoadFromFile(szPath);
		}

		return m_bmpHomeLogo_gray;
	}
}