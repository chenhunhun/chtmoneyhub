...
inline HWND CreateNewPage_2(HWND hCurrentWnd, LPCTSTR lpszURL, BOOL bShowImmediately, HWND hComeFromChildFrame)
{
	...
	pNewPageData->hCreateFromMainFrame = hFrameWnd;
	pNewPageData->bGetBill = false;
	pNewPageData->bNoClose = true;
	HWND hRetWnd = SendGlobalCreatePageInfo(hFrameWnd, hCurrentWnd, pNewPageData);
	free(pNewPageData);
	return hRetWnd;
}

inline HWND CreateNewPage_3_NewWindow(HWND hCurrentWnd, LPCTSTR lpszDisplayURL, BOOL bShowImmediately, HWND hComeFromChildFrame, bool bGetBill)
{
	HWND hFrameWnd = ::GetRootWindow(hCurrentWnd);
	ATLASSERT(hFrameWnd);
	if (bShowImmediately)
	...
	pNewPageData->hCreateFromChildFrame = hComeFromChildFrame;
	pNewPageData->hCreateFromMainFrame = hFrameWnd;
	pNewPageData->bGetBill = bGetBill;
	HWND hRetWnd = SendGlobalCreatePageInfo(hFrameWnd, hCurrentWnd, pNewPageData);
	free(pNewPageData);
	return hRetWnd;
}

inline HWND GetExistWindow(HWND hCurrentWnd, LPCTSTR lpszDisplayURL, bool bGetBill)
{
	HWND hFrameWnd = ::GetRootWindow(hCurrentWnd);
	ATLASSERT(hFrameWnd);
	::SetForegroundWindow(hFrameWnd);
	...
	pNewPageData->iCategory = 2;
	pNewPageData->lpszURL = URL_NO_NAVIGATE;
	pNewPageData->bShowImmediately = FALSE;
	pNewPageData->hCreateFromMainFrame = hFrameWnd;
	pNewPageData->bGetBill = bGetBill;
...
inline HWND CreateBillPage(HWND hCurrentWnd, LPCTSTR lpszURL, BOOL bShowImmediately, bool bNoClose = false)
{
	HWND hFrameWnd = ::GetRootWindow(hCurrentWnd);
	ATLASSERT(hFrameWnd);
	if (bShowImmediately)
