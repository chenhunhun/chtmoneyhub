...
jQuery.validator.addMethod("Different", function(value, element, param) {
	return $("#sAccount").val() != $("#sAccount2").val();
}, jQuery.validator.format("转入账户和转出账户不能相同"));
...
}, jQuery.validator.format("数字不能是0")); 
  
jQuery.validator.addMethod("notNeg", function(value, element, param) {
	var fResult = parseFloat(value);
	if (isNaN(fResult)) {
		return true;
	} else {
