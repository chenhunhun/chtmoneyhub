...
void CBillUrlManager::SendBillRecordToJS(BillData* pData)
{
	...
	//先组原有带入的数据
	char oinfo[2560] = { 0 };
	sprintf_s(oinfo, 2560, "<InputInfo><FinanceId>%s</FinanceId><AccountId>%d</AccountId><Tag>%s</Tag><Type>%d</Type></InputInfo>\r\n", pData->aid.c_str(), pData->accountid, pData->tag.c_str(), pData->type);
	info += oinfo;


	// 组现有的树结构
	sprintf_s(oinfo, 2560, "<OutputInfo><FinanceId>%s</FinanceId><AccountId>%d</AccountId><Tag>%s</Tag><Type>%d</Type></OutputInfo>\r\n<BillInfo>", m_BillRecords.aid, m_BillRecords.accountid, m_BillRecords.tag, m_BillRecords.type);
	info += oinfo;	
